...
import { Clock, Sparkles, Play, Film, Check } from "lucide-react";
...
export function ClipCard({ clip, selecting, selected, onToggle }: ClipCardProps) {
  const inner = (
    <div
      className={cn(
        "glass rounded-2xl overflow-hidden transition-all duration-300 group cursor-pointer",
        selecting ? (selected ? "ring-2 ring-brand-500 glow" : "ring-1 ring-surface-300") : "glass-hover hover:glow"
      )}
      onClick={selecting ? () => onToggle?.(clip.id) : undefined}
    >
      <div className="relative aspect-video bg-surface-100 flex items-center justify-center overflow-hidden">
        ...
        {selecting && (
          <div className={cn(
            "absolute top-2 left-2 z-30 flex h-6 w-6 items-center justify-center rounded-md border-2 transition-all",
            selected ? "gradient-brand border-transparent" : "border-white/60 bg-black/30"
          )}>
      ...
      <div className="p-4 space-y-2">
        <div className="flex items-start justify-between gap-2">
          <h3 className="text-sm font-medium text-surface-800 line-clamp-1">{clip.title}</h3>
          <Badge variant={statusVariant[clip.status]} className="shrink-0">{clip.status}</Badge>
        </div>
        <p className="text-xs text-surface-500 line-clamp-2">{clip.description}</p>
        <div className="flex items-center justify-between pt-1">
          ...
          <span className="text-[10px] text-surface-500">{formatTimeAgo(clip.createdAt)}</span>
        </div>
      </div>
    </div>
  );
  ...
  return <Link href={`/clip/${clip.id}`}>{inner}</Link>;
}
